...
def zapis_priradenia_hestegov(pridadenia_hestegov, db):
    cursor = db.cursor()
    cursor.executemany("INSERT INTO tweet_hashtags (tweet_id, hashtag_id) VALUES (%s, %s) ON CONFLICT DO NOTHING", pridadenia_hestegov)
    db.commit()
...
def spracuj_tvit(db, h, k, mensn, objekt, pouzivatelia, pouzivatelia_update, pridadenia_hestegov, tvity):
    ...
    try:
        ...
        if len(pridadenia_hestegov) > 2000:
            zapis_priradenia_hestegov(pridadenia_hestegov, db)
    except KeyError:
        pass
    pouzivatelia.append(objekt['user'])
    if len(pouzivatelia) > 2000:
        zapis_pouzivatelov(pouzivatelia, db)
    ...
    if len(pouzivatelia_update) > 2000:
        update_pouzivatelov(pouzivatelia_update, mensn, db)
    try:
...
def rob(subor, k, h):
    ...
    spracuj_subor(subor, k, h, connection)
    connection.close()
...
for i in tqdm.tqdm(os.listdir(DATA), ascii=True):
    print("idem spravit: " + DATA + i)
    start = time.time()
    spracuj_subor(DATA + i, krajiny, hestegy, connection)
    end = time.time()
    print(end - start)
connection.close()
# pool.close()
